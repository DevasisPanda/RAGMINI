...
def extract_pdf_pages(file_path: str) -> List[Tuple[str, int]]:
    """Extract text from each PDF page, returning (text, page_number) pairs.
    ...
    try:
        ...
        print(f"PyMuPDF page extraction failed: {e}, trying pdfminer...")
    
    # Fallback to pdfminer
    try:
        from pdfminer.high_level import extract_pages as pdfminer_extract_pages
        from pdfminer.layout import LTTextContainer
        for page_num, page_layout in enumerate(pdfminer_extract_pages(file_path), start=1):
            page_text = ""
...
def extract_text(data: Union[str, Path], show_progress: bool = False) -> str:
    ...
    if isinstance(data, (str, Path)) and os.path.isfile(data):
        ...
        if show_progress and file_size_mb > 1:
            print(f"[INFO] Processing {file_path.name} ({file_size_mb:.1f} MB)...")
        ...
        try:
            ...
            if not text or not text.strip():
                print(f"[WARN] Warning: No content extracted from {file_path.name}")
                return ""
